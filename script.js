...
async function carregarEscolhidos() {
  try {
    ...
    dados.forEach((livro) => {
      ...
      const card = document.querySelector('.livro[data-id="' + id + '"]');
    ...
    if (progresso) {
      const porcentagem = (escolhidos / TOTAL_LIVROS) * 100;
...
function atualizar() {
  ...
  quantidade.innerText = qtd + " livro" + (qtd > 1 ? "s" : "");
...
botao.addEventListener("click", async () => {
  ...
  const nome = document.getElementById("nomePessoa").value || "Não informado";

  const mensagemPessoa = document.getElementById("mensagemPessoa").value;
  ...
  for (const cb of checkboxes) {
    if (cb.checked) {
      ...
      const nomeLivro = card.querySelector("h2").innerText;
  ...
  /* fallback web */
  const whatsappWeb = `https://wa.me/${telefone}?text=${mensagemCodificada}`;
